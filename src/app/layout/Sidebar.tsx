...
import { Map, DoorOpen, PenTool } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = React.memo(({ onHide }) => {
  ...
  const handleNavClick = (view: ViewTab, stageId?: string) => {
    ...
    setPipelineStageId(stageId ?? null);
    setActiveView(view);
  };

  const pipelineItems: { label: string; view: ViewTab; icon: React.FC<{ className?: string }>; badge?: string }[] = [
    { label: 'Plot & Site Area', view: 'plot-area', icon: Map, badge: undefined },
    { label: 'Site + 3D Model', view: 'site-3d', icon: Columns2, badge: 'SPLIT' },
    { label: 'Architectural Plan', view: 'architectural-plan', icon: PenTool },
  ...
  const designItems: { label: string; view: ViewTab; icon: React.FC<{ className?: string }> }[] = [
  ...
  const detailingItems: { label: string; view: ViewTab; icon: React.FC<{ className?: string }> }[] = [
    { label: 'Walls, Doors & Windows', view: 'architectural-plan', icon: DoorOpen },
  ];
  ...
  const outputItems: { label: string; view: ViewTab; icon: React.FC<{ className?: string }> }[] = [
    ...
    { label: 'CAD Drawing Sheets + BBS', view: 'drawings', icon: FileText },
  ...
  return (
    <aside className="w-64 bg-[#0B132B] border-r border-slate-800 flex flex-col h-screen text-slate-300 font-sans z-30 select-none relative shadow-xs">
      ...
      <div className="px-3 py-2 border-b border-slate-800/80 bg-slate-900/40">
        <button
          type="button"
          onClick={() => handleNavClick('dashboard')}
          className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md mb-2 transition-all font-mono text-[11px] font-semibold border ${
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-5 text-xs">
          ...
          {analysisOpen && (
            <ul className="space-y-0.5">
              {pipelineItems.slice(0, 3).map((item) => {
                ...
                const stageId = item.view === 'plot-area' ? 'plot' : item.view === 'site-3d' ? 'plot' : item.view === 'architectural-plan' ? 'architectural' : undefined;
                return (
                  <li key={item.view}>
                    <button
                      onClick={() => handleNavClick(item.view, stageId)}
        ...
        {/* Section 2: Model & Analysis — Stage 3 */}
        <div>
          <div className="flex items-center justify-between px-2 mb-1.5">
            <span className="font-mono text-[10px] text-slate-400 uppercase font-semibold tracking-wider">
          ...
          {analysisOpen && (
            <ul className="space-y-0.5">
              {pipelineItems.slice(2).map((item) => {
                const Icon = item.icon;
                const isActive = activeView === item.view;
                return (
                  <li key={item.view}>
        ...
        {/* Section 3: IS Code Design — Stage 4 */}
        <div>
          <div className="flex items-center justify-between px-2 mb-1.5">
          ...
          {analysisOpen && (
            <ul className="space-y-0.5">
              {detailingItems.map((item) => {
                const Icon = item.icon;
                const isActive = activeView === item.view;
                const stageId = item.view === 'architectural-plan' ? 'detailing' : undefined;
                return (
        ...
        {/* Section 5: Deliverables & Exports — Stage 6 */}
        <div>
          <div className="flex items-center justify-between px-2 mb-1.5">
            <span className="font-mono text-[10px] text-slate-400 uppercase font-semibold tracking-wider">
              STAGE 6 · DRAWINGS & OUTPUTS
            </span>
            <button
              type="button"
